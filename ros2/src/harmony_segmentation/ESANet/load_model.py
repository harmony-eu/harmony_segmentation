...
def main(num_custom_classes, epochs, path, pretrained):
    ...
    if pretrained:
        ...
        weights = torch.load('/home/matteo/Code/deployment_esanet_ros/ESANet/model_best_miou_pt1.pth')
    ...
    train_data = CustomDataset(path_to_train_images, path_to_train_masks, transform=True)
    ...
    loss_weights = torch.tensor([1., 1., 1., 1.]).cuda()
    ...
    for epoch in range(epochs):
        ...
        if ious[1] > best_miou:
            ...
            ckpt_model_filename = "model_best_miou_esa.pth"
            torch.save(state, ckpt_model_filename)
            print('Saved best model.')
        
        state = {'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
        ckpt_model_filename = "model_latest_esa.pth"
        torch.save(state, ckpt_model_filename)
